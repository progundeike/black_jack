...
class Dealer implements Participant
{
    ...
    public function tern(): void
    {
        ...
        echo 'ディーラーの現在の得点は' . $this->card->getSoftHandScore($this->card->hands[$this->name]) . 'です。' . PHP_EOL;

        while ($this->card->getSoftHandScore($this->card->hands[$this->name]) < self::STAND_SCORE) {
